...
public class ImageUtil {
	 ...
	 * 
	 *             Java中也同样是这样。 但java提供了4个缩放的微调选项。 image.SCALE_SMOOTH //平滑优先
	 *             image.SCALE_FAST//速度优先 image.SCALE_AREA_AVERAGING //区域均值
	 *             image.SCALE_REPLICATE //像素复制型缩放 image.SCALE_DEFAULT //默认缩放模式
	 * 
	 *             调用方法 Image new_img=old_img.getScaledInstance(1024, 768,
	 *             Image.SCALE_SMOOTH);
	 */
	public static boolean ratioZoom(File srcFile, String dest, String newFileName, String suffix, int newWidth,
		...
		Image Itemp = bi.getScaledInstance(newWidth, newHeight, BufferedImage.SCALE_REPLICATE);

		// BufferedImage bfImage= new
		// BufferedImage(newWidth,newHeight,BufferedImage.TYPE_INT_RGB);
		// bfImage.getGraphics().drawImage(Bi.getScaledInstance(newWidth,
	 ...
	 */
	public static void zipImageFile(byte[] srcFile, String deskURL, double comBase, double scale) throws Exception {
		InputStream input = new ByteArrayInputStream(srcFile);
	...
	public static void main(String[] args) throws Exception {
		/*try {
			if (ImageUtil.ratioZoom(new File("e:/r1-blue-3.jpg"), "e:/", "aaaa", "jpg", 300, 300)) {
		...
		input.read(byt);
		ImageUtil.zipImageFile(byt, "d:/12345.jpg", 1024d, 1d);
		input.close();*/
		File file = new File("d:/123.jpg");
		InputStream input = new FileInputStream(file);
